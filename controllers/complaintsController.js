  ...
  try {
    ...
    if (type === 'CUSTOM') {
      ...
      complaintRef.child('complaints/').once('value', function (snapshot) {
        ...
        let complaintCount = complaintID.length
        var i = 0
        complaintID.forEach(function (element) {
          complaintRef.child('complaints/' + element).once('value', function (snapshot) {
            ...
            i += 1
            if (i === complaintCount) {
              workbook.xlsx.writeFile(path.join(__dirname, '/Complaints.xlsx')).then(function () {
                console.log('file is written')
                res.download(path.join(__dirname, '/Complaints.xlsx'), function (err, result) {
                  if (err) {
        ...
        complaintID.forEach(function (element) {
          complaintRef.child('complaints/' + element).once('value', function (snapshot) {
            ...
            if (workbook.getWorksheet(months[mon])) {
              // let worksheet = workbook.getWorksheet(months[mon])
            } else {
